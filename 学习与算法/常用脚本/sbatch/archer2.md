...
| 命令       | 主要作用     | 查找对象       | 工作方式               | 典型用途               | 例子                                          |
| -------- | -------- | ---------- | ------------------ | ------------------ | ------------------------------------------- |
| `find`   | 查找文件路径   | 文件、目录      | 实时遍历目录树            | 按文件名、类型、大小、修改时间查文件 | `find . -name "*.c"`                        |
| `locate` | 快速查找文件路径 | 文件、目录      | 从预先建立的索引数据库中搜索     | 快速知道某个文件可能在哪里      | `locate main.c`                             |
| `grep`   | 查找文件中的文字 | 文本内容中的行    | 按关键词或正则匹配整行        | 找包含某个词的行           | `grep "error" log.txt`                      |
| `awk`    | 处理文件中的文字 | 文本内容中的列/字段 | 把每一行拆成字段，再筛选、计算、输出 | 提取列、按条件筛选、统计、格式化输出 | `awk '$2 > 80 {print $1, $2}' students.txt` |

| 对比                 | 结论                                                       |
| ------------------ | -------------------------------------------------------- |
| `find` vs `locate` | 都是找文件路径；`find` 实时但慢，`locate` 查索引所以快，但可能不是最新              |
| `grep` vs `awk`    | 都能处理文本；`grep` 主要负责“找包含关键词的行”，`awk` 主要负责“按列处理、判断、计算和重新输出” |
